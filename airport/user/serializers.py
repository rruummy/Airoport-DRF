...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_passport_number(self, value):
        ...
        if not (is_digit_passport or is_booklet_passport or is_international_passport) or cleaned_value.isalpha():
            raise serializers.ValidationError("Invalid passport data format.")

        qs = UserProfile.objects.filter(passport_number=cleaned_value)

        if getattr(self, 'instance', None):
            qs = qs.exclude(pk=self.instance.pk)

        if qs.exists():
            raise serializers.ValidationError("User with this passport number already exists.")
...
class BalanceSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        def validate_balance(self, value):
            if value < 0:
                raise serializers.ValidationError("The balance cannot be less 0")